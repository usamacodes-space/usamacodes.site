        ...
        <div className="relative rounded-xl p-1 shadow-2xl focus-within:border-[#f97316]/50 focus-within:shadow-[#f97316]/10 focus-within:shadow-2xl transition-all duration-300 bg-[var(--brand-bg)] border border-[#5d707f]/40">
          <div className="flex flex-col">
            <input
              ref={inputRef}
              type="text"
              placeholder="Ask Anything (e.g. experience with Docker?)..."
              className="font-mono bg-transparent border-none outline-none px-3 sm:px-4 py-3 text-[16px] sm:text-[14px] w-full text-[#ecebf3] placeholder:text-[var(--brand-slate-light)] transition-colors touch-manipulation"
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && handleSearch()}
              aria-label="Ask a question about Usama"
            />
            <div className="flex items-center justify-between px-2 pb-1.5 pt-1 border-t border-[#5d707f]/40 mt-1">
              <div className="flex items-center gap-1.5 bg-[#5d707f]/20 rounded-full px-2.5 py-0.5 border border-[#5d707f]/40">
                <Zap className="w-2.5 h-2.5 text-[#f97316]" />
                <span className="font-mono text-[10px] text-[var(--brand-slate-light)] tracking-wider">Groq</span>
              </div>
              <div className="flex items-center gap-1">
                <button type="button" disabled title="Voice input coming soon" className="p-1.5 rounded text-[var(--brand-slate-light)] opacity-60 cursor-not-allowed" aria-label="Voice input (coming soon)">
                  <Mic className="w-3.5 h-3.5" />
                </button>
                <button type="button" onClick={onClear} className="p-1.5 hover:bg-[#5d707f]/30 rounded text-[var(--brand-slate-light)] hover:text-[#f97316] transition-colors" aria-label="Clear and start new question">
                  <RotateCcw className="w-3.5 h-3.5" />
                </button>
                <button
                  onClick={() => handleSearch()}
                  disabled={isTyping}
                  className="font-mono bg-[#f97316] text-white px-4 py-2 sm:py-1 rounded-lg text-[11px] min-h-[44px] sm:min-h-0 flex items-center justify-center gap-2 hover:bg-[#ea580c] hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 disabled:opacity-50 disabled:hover:scale-100 shadow-lg shadow-[#f97316]/20 touch-manipulation"
                >
